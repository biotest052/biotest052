<div align="center">

# i'm biotest05! 

### polish game dev & music producer
<sup>i also mod games, mostly gtag clones to piss them off</sup>

</div>



<div align="center">

### what i do

</div>

- **game development** - ive worked on some released games, check out my website
- **music production** - occasionally making music
- **game modding** - idk i mod some games for fun

<br>

<div align="center">

### what i work on

</div>

* [GTClassic](https://gtclassic.org/) - Community-first, self-hostable fork of Gorilla Tag v1.0.8 + accompanying software.
- [spleece](https://spleece.biotest.dev/) - A free splice sample searcher.
- [voicechat.vc](https://voicechat.vc) - An invite-only (for now) discord inspired messaging app. Its a bit buggy, but were remaking the app & backend.
- [VRCraft](https://vrcraft.dev) - A Minecraft VR clone made in unity.

<div align="center">

### github stats

</div>

<div align="center">

<picture>
  <img src="/github-metrics.svg" alt="Metrics">
</picture>

<br>

</div>

<div align="center">

### find me elsewhere

[![Website](https://img.shields.io/badge/Website-1e1e2e?style=for-the-badge&logo=googlechrome&logoColor=ffffff)](https://biotest.dev/)
[![Youtube](https://img.shields.io/badge/Youtube-1e1e2e?style=for-the-badge&logo=youtube&logoColor=ffffff)](https://www.youtube.com/@biotest05)
[![Steam](https://img.shields.io/badge/Steam-1e1e2e?style=for-the-badge&logo=steam&logoColor=ffffff)](https://steamcommunity.com/id/biotest05/)

</div>

<p align="center">
	<img src="https://github.com/catppuccin/catppuccin/blob/main/assets/palette/macchiato.png?raw=true" />
	<br>
	i have no idea what i'm doing
</p>
